...
func TestRemove(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectErrMsg != "" {
				if err == nil {
					t.Fatalf("Expected error, got nill instead\n")
				}
			...
			if err != nil {
				t.Fatalf("Expected no error, got %q isntead\n", err)
			}
			...
			if hl.Hosts[0] == tc.host {
				t.Errorf("Host name %q should not be int he list\n", tc.host)
			}
...
func TestLoadNoFile(t *testing.T) {
	...
	if err := os.Remove(tf.Name()); err != nil {
		t.Fatalf("Error delting temp file: %s", err)
	}
